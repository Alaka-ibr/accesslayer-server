...
describe('readQueryParam — string type', () => {
   it('returns the string value when present', () => {
      expect(readQueryParam({ q: 'hello' }, 'q', 'string', 'default')).toBe(
         'hello'
      );
   });
   ...
   it('returns defaultValue when param is empty string', () => {
      expect(readQueryParam({ q: '' }, 'q', 'string', 'default')).toBe(
         'default'
      );
   });

   it('returns defaultValue when param is null', () => {
      expect(readQueryParam({ q: null }, 'q', 'string', 'fallback')).toBe(
         'fallback'
      );
   });
   ...
   it('uses first element of array value', () => {
      expect(
         readQueryParam({ q: ['first', 'second'] }, 'q', 'string', 'x')
      ).toBe('first');
   });
...
describe('readQueryParam — boolean type', () => {
   ...
   it('uses first element of array value', () => {
      expect(
         readQueryParam({ b: ['true', 'false'] }, 'b', 'boolean', false)
      ).toBe(true);
   });
});
