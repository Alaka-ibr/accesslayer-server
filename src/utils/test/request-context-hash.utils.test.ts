...
describe('computeRequestContextHash()', () => {
   ...
   it('returns the same hash for identical inputs', () => {
      ...
      expect(computeRequestContextHash(req)).toBe(
         computeRequestContextHash(req)
      );
   });
   ...
   it('produces different hashes for different HTTP methods', () => {
      ...
      expect(computeRequestContextHash(get)).not.toBe(
         computeRequestContextHash(post)
      );
   });
   ...
   it('produces different hashes for different paths', () => {
      ...
      expect(computeRequestContextHash(a)).not.toBe(
         computeRequestContextHash(b)
      );
   });
   ...
   it('produces different hashes for different content-type headers', () => {
      ...
      expect(computeRequestContextHash(json)).not.toBe(
         computeRequestContextHash(form)
      );
   });
   ...
   it('ignores query strings in the path', () => {
      ...
      expect(computeRequestContextHash(withQuery)).toBe(
         computeRequestContextHash(withoutQuery)
      );
   });
   ...
   it('produces the same hash regardless of Authorization header value', () => {
      ...
      const withAuth = makeReq({
         path: '/api/creators',
         headers: { authorization: 'Bearer secret-token' },
      });
      expect(computeRequestContextHash(noAuth)).toBe(
         computeRequestContextHash(withAuth)
      );
   });

   it('produces the same hash regardless of Cookie header value', () => {
      const noCookie = makeReq({ path: '/api/creators', headers: {} });
      const withCookie = makeReq({
         path: '/api/creators',
         headers: { cookie: 'session=abc123' },
      });
      expect(computeRequestContextHash(noCookie)).toBe(
         computeRequestContextHash(withCookie)
      );
   });
   ...
   it('treats content-type with and without charset as the same', () => {
      ...
      const withCharset = makeReq({
         method: 'POST',
         path: '/api/auth',
         headers: { 'content-type': 'application/json; charset=utf-8' },
      });
      expect(computeRequestContextHash(plain)).toBe(
         computeRequestContextHash(withCharset)
      );
   });
   ...
   it('falls back to "/" when path is empty', () => {
      ...
      expect(computeRequestContextHash(emptyPath)).toBe(
         computeRequestContextHash(rootPath)
      );
   });
   ...
   it('is case-insensitive for method (get vs GET)', () => {
      ...
      expect(computeRequestContextHash(upper)).toBe(
         computeRequestContextHash(lower)
      );
   });
});
