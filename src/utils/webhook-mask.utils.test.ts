...
describe('maskWebhookUrl', () => {
   it('strips query string containing a secret', () => {
      expect(maskWebhookUrl('https://api.example.com/hook?token=secret')).toBe(
   ...
   it('returns the URL as-is when there is no path or query string', () => {
      expect(maskWebhookUrl('http://localhost:3000')).toBe(
         'http://localhost:3000'
      );
   });
   ...
   it('strips path and preserves non-standard port', () => {
      expect(
         maskWebhookUrl('https://internal.corp:8443/webhooks/receiver?auth=abc')
      ).toBe('https://internal.corp:8443');
   });

   it('strips fragment identifiers', () => {
      expect(maskWebhookUrl('https://example.com/path#section')).toBe(
         'https://example.com'
      );
   });
});
