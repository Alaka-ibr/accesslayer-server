...
async function run() {
   ...
   assert.equal(
      sig1,
      sig2,
      'Signatures should be identical for same params in different order'
   );
   ...
   assert.equal(
      sigSensitive,
      sigSafe,
      'Sensitive fields should be excluded from signature'
   );
   ...
   assert.equal(
      emptySig1,
      emptySig2,
      'Empty query should return consistent signature'
   );
   assert.ok(
      typeof emptySig1 === 'string' && emptySig1.length === 64,
   ...
   assert.ok(
      typeof complexSig === 'string' && complexSig.length === 64,
      'Complex values should be handled'
   );
