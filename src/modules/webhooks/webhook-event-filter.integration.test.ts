...
describe('Webhook event type filter — sell-only webhook', () => {
   ...
   afterEach(async () => {
      await prisma.webhookEvent.deleteMany({
         where: { webhookId: sellOnlyWebhookId },
      });
...
describe('Webhook event type filter — cross-isolation', () => {
   it('buy-only webhook and sell-only webhook co-exist — each receives only their event', async () => {
      const buyWh = await prisma.webhook.create({
         data: {
            creatorId,
            callbackUrl: 'https://example.com/buy',
            events: { set: ['BUY'] },
         },
      });
      const sellWh = await prisma.webhook.create({
         data: {
            creatorId,
            callbackUrl: 'https://example.com/sell',
            events: { set: ['SELL'] },
         },
      });
      ...
      const buyEvents = await prisma.webhookEvent.findMany({
         where: { webhookId: buyWh.id },
      });
      const sellEventsOnBuyFire = await prisma.webhookEvent.findMany({
         where: { webhookId: sellWh.id },
      });
      ...
      const sellEvents = await prisma.webhookEvent.findMany({
         where: { webhookId: sellWh.id },
      });
      ...
      // cleanup
      await prisma.webhookEvent.deleteMany({
         where: { webhookId: { in: [buyWh.id, sellWh.id] } },
      });
      await prisma.webhook.deleteMany({
         where: { id: { in: [buyWh.id, sellWh.id] } },
      });
   });
});
