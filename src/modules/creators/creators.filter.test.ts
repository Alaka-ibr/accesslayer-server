...
function run() {
   ...
   assert.deepEqual(parseCreatorFilters({ search: 'jazz   musician' }), {
      search: 'jazz musician',
   });
   ...
   assert.deepEqual(parseCreatorFilters({ search: 'alice' }), {
      search: 'alice',
   });

   // --- verified: coercion ---

   assert.deepEqual(parseCreatorFilters({ verified: 'true' }), {
      verified: true,
   });
   assert.deepEqual(parseCreatorFilters({ verified: 'false' }), {
      verified: false,
   });
   assert.deepEqual(parseCreatorFilters({ verified: '1' }), { verified: true });
   assert.deepEqual(parseCreatorFilters({ verified: '0' }), {
      verified: false,
   });
   assert.deepEqual(parseCreatorFilters({ verified: true }), {
      verified: true,
