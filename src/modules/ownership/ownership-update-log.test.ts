...
describe('updateOwnership — structured debug log', () => {
   const ownerAddress =
      'GTEST1234GTEST1234GTEST1234GTEST1234GTEST1234GTEST12345';
   ...
   it('emits debug log with all six required fields after successful update', async () => {
      (mockPrisma.keyOwnership.findFirst as jest.Mock).mockResolvedValue(
         mockOwnership(10)
      );
      (mockPrisma.keyOwnership.upsert as jest.Mock).mockResolvedValue(
         mockOwnership(15)
      );
   ...
   it('masks wallet address to first 4 and last 4 characters', async () => {
      ...
      expect(
         logFields.wallet_address.startsWith(ownerAddress.slice(0, 4))
      ).toBe(true);
      expect(logFields.wallet_address.endsWith(ownerAddress.slice(-4))).toBe(
         true
      );
   });
   ...
   it('does not emit log when upsert throws', async () => {
      (mockPrisma.keyOwnership.findFirst as jest.Mock).mockResolvedValue(null);
      (mockPrisma.keyOwnership.upsert as jest.Mock).mockRejectedValue(
         new Error('DB error')
      );

      await expect(updateOwnership(ownerAddress, creatorId, 5)).rejects.toThrow(
         'DB error'
      );
