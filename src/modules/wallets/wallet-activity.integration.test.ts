...
function makeActivity(
   overrides: Partial<WalletActivityItem> = {}
): WalletActivityItem {
   return {
      id: 'activity-1',
      type: 'buy',
...
describe('GET /wallets/:address/activity', () => {
   ...
   it('returns 200 with items and meta for a wallet with mixed trade history', async () => {
      ...
      jest
         ...
         .mockResolvedValue([activities, 2, null]);
   ...
   it('each item includes required trade fields', async () => {
      ...
      jest
         .spyOn(walletActivityService, 'fetchWalletActivity')
         .mockResolvedValue([[activity], 1, null]);
   ...
   it('returns only buy events when type=buy', async () => {
      ...
      const body = res.json.mock.calls[0][0];
      expect(
         body.data.items.every((i: WalletActivityItem) => i.type === 'buy')
      ).toBe(true);
   });
   ...
   it('returns only trades for the specified creator when creator_id is set', async () => {
      ...
      jest
         .spyOn(walletActivityService, 'fetchWalletActivity')
         .mockResolvedValue([items, 1, null]);
      ...
      expect(
         body.data.items.every(
            (i: WalletActivityItem) => i.creator_id === 'creator-abc'
         )
      ).toBe(true);
   });
   ...
   it('meta reflects limit and offset correctly', async () => {
      ...
      jest
         .spyOn(walletActivityService, 'fetchWalletActivity')
         .mockResolvedValue([items, 50, null]);
