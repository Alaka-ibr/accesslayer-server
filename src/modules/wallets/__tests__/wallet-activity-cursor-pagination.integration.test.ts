...
describe('wallet activity feed cursor-based pagination integration', () => {
   ...
   it('returns a valid next cursor on the first page', async () => {
      ...
      const nextCursorStr = encodeCursor<ActivityFeedCursorPayload>({
         id: lastOnPageOne.id,
      });
   ...
   it('fetches the second page using the cursor and confirms no duplicates', async () => {
      ...
      jest
         .spyOn(walletActivityService, 'fetchWalletActivity')
         .mockResolvedValue([PAGE_TWO_FIXTURES, ALL_FIXTURES.length, null]);

      const reqTwo = makeReq(
         { address: WALLET_ADDRESS },
         { limit: '3', cursor: nextCursorStr }
      );
