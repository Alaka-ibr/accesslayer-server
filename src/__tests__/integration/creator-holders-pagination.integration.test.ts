...
describe('GET /api/v1/creators/:id/holders pagination', () => {
   ...
   it('paginates correctly across multiple pages', async () => {
      ...
      while (hasMore) {
         const res = await supertest(app).get(
            `/api/v1/creators/${creatorId}/holders?limit=${PAGE_SIZE}&offset=${offset}`
         );
      ...
      const overlapOneTwo = pageOneIds.filter((id: string) =>
         pageTwoIds.includes(id)
      );
      const overlapTwoThree = pageTwoIds.filter((id: string) =>
         pageThreeIds.includes(id)
      );
      const overlapOneThree = pageOneIds.filter((id: string) =>
         pageThreeIds.includes(id)
      );
   ...
   it('final page returns hasMore=false', async () => {
      ...
      const res = await supertest(app).get(
         `/api/v1/creators/${creatorId}/holders?limit=${PAGE_SIZE}&offset=${lastOffset}`
      );
